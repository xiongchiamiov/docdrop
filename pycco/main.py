...
def parse(sourceFilename, code):
    ...
    lines = code.split("\n")
    sections = []
    language = get_language(sourceFilename, code)
...
def preprocess(comment, section_nr, preserve_paths=True):
    """
...
def highlight(source, code, sections, preserve_paths=True):
    """
...
def generate_html(source, sections, preserve_paths=True):
    """
    ...
    dest = destination(source, preserve_paths=preserve_paths)
    #csspath = path.relpath(path.join(outdir, "pycco.css"), path.split(dest)[0])
    csspath = '/static/pycco.css'
...
def destination(filepath, preserve_paths=True):
    """
